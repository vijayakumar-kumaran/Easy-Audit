  ...
  const fetchNotices = async () => {
    try {
      ...
      if (user.userType === 'audituser') {
        response = await axios.get('/api/bus-it-notices', {
          params: { username: user.username }
        });
      } else {
        response = await axios.get('/api/bus-it-notices-all');
      }
      setNotices(response.data);
      setLoading(false);
    } catch (error) {
      console.error('Error fetching business IT notices:', error);
      setLoading(false);
    }
  };

  const filteredNotices = notices.filter(notice => {
    const clientName = notice.businessName?.toLowerCase() || '';
    ...
    <div className="customer-view-page fade-in">
      <div className="view-page-header">
        <div className="header-meta">
          <button className="back-button" onClick={() => navigate('/business-notices')}>
            <ArrowLeft size={16} />
            <span>Workflow Dashboard</span>
          </button>
          <h1>Business IT Notice Tasks</h1>
          <p>Track response statuses, due dates, and document queries for registered companies.</p>
        </div>
      </div>
      ...
      <div className="search-filter-card glass-card" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
        <div className="search-wrapper-custom" style={{ flexGrow: 1, minWidth: '250px' }}>
          ...
          <input
            type="text"
            placeholder="Search by company name, notice type, or officer..."
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="no-records-card glass-card">
          ...
          <p>No business IT notice tasks match the selected search filters.</p>
        </div>
      ) : (
              ...
              {filteredNotices.map((notice, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td>
                      <span>{notice.businessName || 'N/A'}</span>
                    </td>
                    <td>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <Link 
                        to={`/update-bus-it-notice/${notice.id}`} 
