...
import { Search, ArrowLeft, Download, Eye, Edit, AlertCircle, FileText } from "lucide-react";
import axios from "axios";
import { useAuth } from "../AuthContext";
import '../css/customer_list.css'; // Reuses directory table styles
import '../css/workflows.css'; // Reuses status badge styles
...
const ListTaxAssessments = () => {
  ...
  const fetchTaxAssessments = async () => {
    try {
      ...
      if (user.userType === 'audituser') {
        response = await axios.get("/api/customer-tax-assessments", {
          params: { username: user.username }
        });
      } else {
        response = await axios.get("/api/customer-tax-assessments-all");
      }
  ...
  const filteredAssessments = taxAssessments.filter(assessment => {
    const clientName = assessment.assesseName?.toLowerCase() || '';
    ...
    <div className="customer-view-page fade-in">
      <div className="view-page-header">
        <div className="header-meta">
          <button className="back-button" onClick={() => navigate('/tax-assesment')}>
            <ArrowLeft size={16} />
            <span>Workflow Dashboard</span>
          </button>
          <h1>Individual Tax Assessments</h1>
          <p>Track audit task assignments, verify documents, and update statuses.</p>
        </div>
      </div>
      ...
      <div className="search-filter-card glass-card" style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
        <div className="search-wrapper-custom" style={{ flexGrow: 1, minWidth: '250px' }}>
          ...
          <input
            type="text"
            placeholder="Search by client name or assigned officer..."
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="no-records-card glass-card">
          ...
          <p>No individual tax assessment tasks match the selected search filters.</p>
        </div>
      ) : (
            ...
            <tbody>
              {filteredAssessments.map((assessment, index) => {
                ...
                return (
                  ...
                  >
                    <td>
                      <span style={{ fontWeight: 600 }}>{assessment.assesseName}</span>
                    </td>
                    <td>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <Link 
                        to={`/update-tax-assessment/${assessment.id}`} 
