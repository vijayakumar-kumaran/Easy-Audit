...
const EditITNotice = () => {
  ...
  useEffect(() => {
    const fetchNoticeData = async () => {
      try {
        const response = await axios.get(`/api/customer-it-notices/${id}`);
    ...
    const fetchNoticeTypes = async () => {
      try {
        const response = await axios.get('/api/notice-types');
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('dob', new Date(notice.dob).toISOString().split('T')[0]);
    ...
    try {
      await axios.put(`/api/customer-it-notices/${id}`, formData, {
  ...
  return (
    <div className="workflow-form-page fade-in">
      ...
      <div className="workflow-layout-grid">
        {/* Left Side: Notice & Client Info Summary */}
        <div className="form-client-read-only-section glass-card">
          ...
          <div className="read-only-info-list">
            <div className="read-only-item">
              ...
              <span className="read-only-value">{notice.assesseeName}</span>
            </div>
            <div className="read-only-item">
            ...
            <div className="read-only-item">
              ...
              <span className="read-only-value" style={{ fontFamily: 'monospace' }}>{notice.panNumber || 'N/A'}</span>
            </div>
            <div className="read-only-item">
              <span className="read-only-label">Phone Connection</span>
              <span className="read-only-value">{notice.phoneNumber || 'N/A'}</span>
            </div>
            {notice.comments && (
              ...
              <>
                ...
                <div className="form-group-custom">
                  ...
                  <textarea 
                    ...
                    required 
                    placeholder="Document filings, replies submitted, compliance summaries..."
                    className="form-control-custom"
                    style={{ minHeight: '80px', resize: 'vertical' }}
                  />
                </div>
